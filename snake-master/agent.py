...
class Agent:
    ...
    #initilazie snake full body (head, body, tail. Its a map containing all body arts with head = 1 and tail = 1 of the snake. Its to track all parts.
    snake = {}
    ...
    def get_move(self, board, score, turns_alive, turns_to_starve, direction):
        ...
        #scans the board for width, height, head, tail, body, food, walls. slef.scan(board) returns the values for all those variable
        ...
        """"SEARCH FOR FOOD"""
        """"agent has a list with all coordinates of the food because of previous scan/function self.scan(board)
            it enters the for-loop. For each food within food-list it checks with A* which is the best path
            this is the final selection og finding a path
            !!searchs for shortest move by calling the A* function!!
            1) checks is path exists and your not dead before your reach that food unit (nothing is in thte way) it will take it always (new_path) for one specific unit in the food list (first one in the food list)
            1.2) checks if from that path that was just found (food unit) there exists a way to one of the (two) other food units
            2) checks the ath for the second and third  in the lsit and checks if path exists and if thispath is shorter that the other and in case it will take that path
            3) if finds a part or DOES NOT: than it will select GO RANDOM -> see go dandom

        """
        for unit in food:
            new_path = self.a_search_shortest(snake_head, unit, board_width, board_height, snake_head, snake_body,  #returns a full path to the food if it exists
                                              walls)
        ...
        """"Go random"""
        """"
        Looks at the neighbores (4) of the snake head and picks the one with the biggest heuristic values: sums the values from all three food units up. Which is the farest away in total. 
        ...
        if not move:
            ...
            new_h = 0
            old_h = 0
            for adj in self.get_adj(snake_head,board_width,board_height,snake_head,snake_body,walls):   #returns all neighbores of coordiante
                if 0 < adj[0] < board_width and 0 < adj[1] < board_width:                               #try to keep bourder free
                    ...
                    if new_h > old_h:                                                                   # checks on all and selects the one woth the highest
                        path = [snake_head, adj]
            # chekc if there is a move to from that highest neighbore, and if this is still not the case it selects a rondom existing one
            if path:
                move = self.get_next_move(path,direction)
    ...
    # check sposition of each object on the board: board is a list fild with a map and its coordiantes and the objects which are connected with a cooridnate
    # list is the width and the maps is the height of the board
    def scan_board(self, board):
        ...
        food = []
        walls = []
        # search through the whole lsit like in the the 4 in a row game. if it is equal food, add it to the food list etc.
        for x in range(board_width+1):
            for y in range(board_height+1):
                ...
                if board[x][y] == GameObject.WALL:
                    walls.append((x, y))
    # checks for every part in the snake lst, except the head which is in its own tuple (x,y). you check how long a part of the snake has been in the coordinate while the snake did one move furtehr
...
# reutn the absolut value  of the heuristic value of x ad y to get a final result to calculate with, every step is +1
...
# gets the path, but just does the first move within the calucalted path. Calculate snext coordinate minus current coordinate
    #translate your move to the given variables which are festgelegt in other .py
    #
    ...
    # get all neighboures of the current state which are availibe and do not lead into the body, wall (units in the way) , head or border
